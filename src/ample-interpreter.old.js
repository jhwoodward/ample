  ...
  while (cursor < player.score.length) {
    ...
    if (isStartKeySwitch(char)) {
      ...
      if (keyswitch.temp) {
        ...
        } else if (player.annotations.default) {
          var defaultKeyswitch = getKeyswitch(player.annotations.default, 0);
    ...
    var rest = isRest(char);
    var note = isNote(char);

    // ************* SEND NOTE ********************** //
    if ((note || rest || isRelativePitch(char))) {
      sendNote(lastNote);
    }
    // *********************************************** //
    ...
    if (lastNote &&
      lastNote.previous &&
      lastNote.previous.legato &&
      !phrase.legato &&
      !lastNote.legato &&
      !lastNote.previous.noteOffSent) {
      sendNoteOff(lastNote.previous, tickCount - lastNote.previous.duration);
    }
